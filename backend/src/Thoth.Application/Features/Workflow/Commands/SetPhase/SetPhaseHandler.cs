...
public sealed class SetPhaseHandler(
    IApplicationDbContext context,
    IWorkflowNotifier workflowNotifier,
    ICurrentUser currentUser,
    IDateTimeProvider dateTimeProvider)
    : IRequestHandler<SetPhaseCommand, WorkflowDto>
{
    public async Task<WorkflowDto> Handle(SetPhaseCommand request, CancellationToken cancellationToken)
    {
        ...
        var target = phases.FirstOrDefault(phase => phase.Id == request.PhaseId)
            ?? throw new NotFoundException("Phase was not found.");

        var now = dateTimeProvider.UtcNow;
        var actor = request.Actor ?? target.DefaultActor;
        WorkflowMutationHelpers.EnterPhase(workflow, target, actor, now);
