...
public sealed class AgentUsageReaderTests : IDisposable
{
    ...
    [Fact]
    public async Task Grok_reader_returns_no_data_without_budget()
    {
        var logsDir = Path.Combine(_root, ".grok", "logs");
        Directory.CreateDirectory(logsDir);
        await File.WriteAllTextAsync(Path.Combine(logsDir, "unified.jsonl"), CreateGrokLogLine(_clock.UtcNow.AddMinutes(-10), 1000, 200, 300));
        var reader = CreateGrokReader(logsDir, fiveHourTokens: 0, weeklyTokens: 0);
    ...
    [Fact]
    public async Task Grok_reader_returns_no_data_when_directory_missing()
    {
        var reader = CreateGrokReader(Path.Combine(_root, "nonexistent-logs"), fiveHourTokens: 100_000, weeklyTokens: 700_000);
    ...
    [Fact]
    public async Task Grok_reader_sums_tokens_in_windows()
    {
        var logsDir = Path.Combine(_root, ".grok", "logs");
        Directory.CreateDirectory(logsDir);

        var recentLine = CreateGrokLogLine(_clock.UtcNow.AddHours(-1), promptTokens: 1000, cachedTokens: 400, completionTokens: 200);
        var oldLine = CreateGrokLogLine(_clock.UtcNow.AddDays(-3), promptTokens: 2000, cachedTokens: 500, completionTokens: 300);
        await File.WriteAllTextAsync(Path.Combine(logsDir, "unified.jsonl"), oldLine + "\n" + recentLine);

        var reader = CreateGrokReader(logsDir, fiveHourTokens: 10_000, weeklyTokens: 100_000);
        ...
        var fiveHour = result.Windows.First(w => w.Key == "five_hour");
        fiveHour.UsedTokens.Should().Be(800);
        fiveHour.LimitTokens.Should().Be(10_000);
        fiveHour.Estimated.Should().BeTrue();

        var sevenDay = result.Windows.First(w => w.Key == "seven_day");
        sevenDay.UsedTokens.Should().Be(2600);
        sevenDay.LimitTokens.Should().Be(100_000);
        sevenDay.Estimated.Should().BeTrue();
    }

    [Fact]
    public async Task Grok_reader_ignores_lines_outside_seven_day_window()
    {
        var logsDir = Path.Combine(_root, ".grok", "logs");
        Directory.CreateDirectory(logsDir);

        var veryOldLine = CreateGrokLogLine(_clock.UtcNow.AddDays(-10), promptTokens: 5000, cachedTokens: 0, completionTokens: 1000);
        await File.WriteAllTextAsync(Path.Combine(logsDir, "unified.jsonl"), veryOldLine);
        ...
        result.Status.Should().Be(AgentUsageStatus.Ok);
        var sevenDay = result.Windows.First(w => w.Key == "seven_day");
        sevenDay.UsedTokens.Should().Be(0);
    }

    [Fact]
    public async Task Grok_reader_ignores_malformed_lines()
    {
        var logsDir = Path.Combine(_root, ".grok", "logs");
        Directory.CreateDirectory(logsDir);

        var validLine = CreateGrokLogLine(_clock.UtcNow.AddMinutes(-30), promptTokens: 500, cachedTokens: 100, completionTokens: 100);
        await File.WriteAllTextAsync(
            Path.Combine(logsDir, "unified.jsonl"),
            "not-json\n{\"invalid\":true}\n" + validLine);
        ...
        result.Status.Should().Be(AgentUsageStatus.Ok);
        var fiveHour = result.Windows.First(w => w.Key == "five_hour");
        fiveHour.UsedTokens.Should().Be(500);
    }

    private static string CreateGrokLogLine(DateTimeOffset ts, long promptTokens, long cachedTokens, long completionTokens) =>
    ...
    private GrokUsageReader CreateGrokReader(string logsDir, long fiveHourTokens, long weeklyTokens)
    {
        var opts = Options.Create(new AgentUsageOptions
        {
            Grok =
            {
                LogPath = logsDir,
        ...
        return new GrokUsageReader(opts, CreateCache(), _clock);
    }
